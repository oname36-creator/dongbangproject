...
- `Client/` — 이 게임 고유의 콘텐츠/로직 + Win32 엔트리포인트. `DongbangProject.cpp/h/.rc/.ico`, `small.ico`(WinMain, 윈도우 생성, 메시지 루프, 리소스 스크립트 — 원본 "Game1945" 네이밍에서 전부 교체됨), `Game`(최상위 오케스트레이터), `Scene`(플레이 필드의 모든 Actor 소유), `Player`/`Enemy`(둘 다 `Airplane` 상속), `Bullet`, `Effect`, `Background`/`WorldBG`, `UIManager`, `ResourceData`(이 게임 전용 에셋 JSON 스키마). **동방프로젝트 컨셉 작업(캐릭터, 탄막 패턴 등)은 주로 이 폴더에서 진행하게 된다.** Actor를 상속하는 구체 클래스들(`Airplane`, `Player`, `Enemy`, `Boss`, `Bullet`, `Effect`, `Background`/`WorldBG`, `Item`)은 디스크상으로는 그냥 `Client/` 밑에 있지만, `DongbangProject.vcxproj.filters`에서 `Client\Actors`라는 가상 필터로 묶여 있어 Visual Studio 솔루션 탐색기에서는 따로 모여 보인다(실제 폴더 이동은 아님).
